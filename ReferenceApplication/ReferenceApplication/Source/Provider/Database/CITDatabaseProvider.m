...
@implementation CITDatabaseProvider

+ (CITDatabaseProvider *)sharedProvider {
    static CITDatabaseProvider *_sharedManager = nil;
...
{
    [databaseMigrationSchemas addObject:databaseSchemaMigration];
}
...
- (void)createTables
{
    ...
    [databaseQueue inTransaction:^(FMDatabase* db, BOOL *rollback){
        ...
        NSString *userVersion = [NSString stringWithFormat:@"PRAGMA user_version = %ld;", [databaseSchema schemaVersion]];
        ...
        [databaseQueue inTransaction:^(FMDatabase *db, BOOL *rollback){
            ...
            for (id <CITDatabaseSchemaMigration> migrationSchema in databaseMigrationSchemas) {
                if (userVersion < [migrationSchema newVersion]) {
                    ...
                    NSString *userVersion = [NSString stringWithFormat:@"PRAGMA user_version = %ld;", [migrationSchema newVersion]];
                    [db executeUpdate:userVersion];
                    
                    for (NSString *sqlQuery in [(id<CITDatabaseSchema>) migrationSchema createTableQueries]) {
                        [db executeUpdate:sqlQuery];
                        [db closeOpenResultSets];
                    }
                    
                    if ([(id<CITDatabaseSchema>) migrationSchema respondsToSelector:@selector(runMigrationWithDatabase:)]) {
                        [(id<CITDatabaseSchema>) migrationSchema runMigrationWithDatabase:db];
                    }
                }
            }
            [databaseMigrationSchemas removeAllObjects];
        }];
